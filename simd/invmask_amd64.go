// Code generated from " ../gtl/generate.py --prefix=Invmask -DOPCHAR=&^ --package=simd --output=invmask_amd64.go invmask_amd64.go.tpl ". DO NOT EDIT.
...
// InvmaskUnsafeInplace sets main[pos] := arg[pos] &^ main[pos] for every position
// in main[].
//
...
// InvmaskInplace sets main[pos] := arg[pos] &^ main[pos] for every position in
// main[].  It panics if slice lengths don't match.
func InvmaskInplace(main, arg []byte) {
	// This takes ~6-8% longer than InvmaskUnsafeInplace on the short-array benchmark
	// on my Mac.
	mainLen := len(main)
	if len(arg) != mainLen {
