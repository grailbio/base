...
func newDigest(h crypto.Hash, b []byte) Digest {
	d := Digest{h: h}
	copy(d.b[:], b)
	return d
}
...
func (d Digester) FromBytes(p []byte) Digest {
	...
	return newDigest(crypto.Hash(d), w.Sum(nil))
}
...
// Rand returns a random digest generated by a cryptographically
// secure random number generator.
func (d Digester) Rand() Digest {
	dg := Digest{h: crypto.Hash(d)}
	if _, err := rand.Read(dg.b[:dg.h.Size()]); err != nil {
		panic(err)
	}
	return dg
}
...
func (d Writer) Digest() Digest {
	return newDigest(d.h, d.w.Sum(nil))
}
